...
genos = read_plink(paste(opt$ref_ld_chr,sep=''),impute="avg")
...
for ( w in 1:nrow(wgtlist) ) {
	...
	if (!file.exists(wgt.file)) {
		cat(paste0("WARNING : weight file does not exist: ",wgt.file))
		next
	}
	load(wgt.file)
	...
	m.keep = !is.na(m)
	snps = snps[m.keep,]
	if (is.null(dim(wgt.matrix))) {
	       	cat(paste0("WARNING : wgt.matrix for gene is invalid: ",wgt.file))
		next
	}
	...
	# added this section so that it will select the best model by R2 if there are no p-values for the model	
	if ( length(row.pval) == 0 ) {
		mod.best = which.max(
        	apply(cv.performance[row.rsq,,drop=FALSE], 2, mean, na.rm = TRUE))
	} else {
    		mod.best = which.min(
        	apply(cv.performance[row.pval,,drop=FALSE], 2, min, na.rm = TRUE))
	}
	...
	if ( length(cur.Z) == 0 ) {
		...
		if ( !cur.FAIL ) {
			...
			#write.table(cur.LD,"../data/TWASResults_trans/LD_matrix.txt",sep = "\t",col.names = T, row.names = T)
